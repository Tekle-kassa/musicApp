...
export const createSongSlice = createSlice({
    ...
    reducers: {
        createSongAction: (state, {payload:formSong}:PayloadAction<FormData>) => {
            state.isLoading = true;
            state.errors = "";
        },
        createSongSuccessAction: (
            state, { payload: status }: PayloadAction<boolean>
        ) => {
...
export const { createSongAction, createSongSuccessAction, createSongErrorAction } =
